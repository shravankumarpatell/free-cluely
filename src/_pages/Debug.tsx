  ...
  return (
    <div className="space-y-1.5">
      ...
      {isLoading ? (
        ...
        <div className={`flex flex-row gap-0.5 rounded-lg overflow-hidden border ${
          isDarkTheme 
            ? 'bg-black/40 backdrop-blur-md border-white/10'
            : 'bg-white/60 backdrop-blur-md border-gray-300'
        }`}>
          {/* Previous Code */}
...
const Debug: React.FC<DebugProps> = ({ isProcessing, setIsProcessing, isDarkTheme, onThemeToggle }) => {
  ...
  useEffect(() => {
    ...
    const cleanupFunctions = [
      ...
      window.electronAPI.onDebugSuccess(() => {
        setIsProcessing(false) //all the other stuff happens in the parent component, so we just need to do this.
      }),
  ...
  return (
    <div className={`${isDarkTheme ? '' : 'theme-light'}`}>
      <div ref={contentRef} className="relative space-y-3 px-4 py-3">
        ...
        {/* Main Content - Updated with theme support */}
        <div className={`w-full text-sm rounded-md ${
          isDarkTheme 
            ? 'text-white/90 main-content-bg'
