    ...
    {isLoading ? (
      <div className="mt-4 flex">
        <p className={`text-xs animate-pulse ${
          isDarkTheme 
            ? 'bg-gradient-to-r from-white/40 via-white/60 to-white/40 bg-clip-text text-transparent'
            : 'bg-gradient-to-r from-gray-500 via-gray-700 to-gray-500 bg-clip-text text-transparent'
        }`}>
          Extracting problem statement...
        </p>
      </div>
    ) : (
      <div className={`text-[13px] leading-[1.4] max-w-[600px] ${
...
const SolutionSection = ({
  ...
  // Copy functionality for code blocks only
    ...
    {isLoading ? (
      ...
      <div className="space-y-1">
        <div className={`flex items-start gap-2 text-[13px] leading-[1.4] ${
          ...
          <div>
            <strong className={isDarkTheme ? 'text-white/90' : 'text-gray-900'}>Time:</strong> {timeComplexity}
          </div>
        </div>
        <div className={`flex items-start gap-2 text-[13px] leading-[1.4] ${
          isDarkTheme ? 'text-white/80' : 'text-gray-700'
          ...
          <div>
            <strong className={isDarkTheme ? 'text-white/90' : 'text-gray-900'}>Space:</strong> {spaceComplexity}
          </div>
        </div>
      </div>
    )}
...
const Solutions: React.FC<SolutionsProps> = ({ setView, isDarkTheme, onThemeToggle }) => {
  const queryClient = useQueryClient()
  const contentRef = useRef<HTMLDivElement>(null)
  ...
  return (
    <div className={`${isDarkTheme ? '' : 'theme-light'}`}>
      {!isResetting && queryClient.getQueryData(["new_solution"]) ? (
        <>
          <Debug
            isProcessing={debugProcessing}
        ...
        <div ref={contentRef} className="relative space-y-3 px-4 py-3">
          ...
          {/* Main Content - Modified width constraints with theme support */}
          <div className={`w-full text-sm rounded-md ${
            isDarkTheme 
              ? 'text-white main-content-bg'
              : 'text-gray-800 bg-white/60 backdrop-blur-md border border-gray-300'
          }`}>
            <div className="rounded-lg overflow-hidden">
              <div className={`px-4 py-3 space-y-4 max-w-full ${
                                ...
                                <div className="space-y-1">
                                  {thoughtsData.map((thought, index) => (
                                    ...
                                    >
                                      ...
                                      <div className={isDarkTheme ? 'text-white/80' : 'text-gray-700'}>{thought}</div>
                                    </div>
                                  ))}
                                </div>
                              </div>
