...
// Enhanced markdown parser with BLOCK-LEVEL copy functionality only
const parseMarkdown = (text: string, isDarkTheme: boolean): JSX.Element => {
  const lines = text.split('\n');
  const elements: JSX.Element[] = [];
  let currentCodeBlock: string[] = [];
  let inCodeBlock = false;
  ...
  // Function to copy code to clipboard - ONLY for code blocks
  const copyToClipboard = (code: string) => {
    navigator.clipboard.writeText(code).then(() => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim().startsWith('```')) {
      if (!inCodeBlock) {
        ...
        // Ending a code block - ONLY place where copy functionality appears
        inCodeBlock = false;
        const codeContent = currentCodeBlock.join('\n');
        elements.push(
          <div key={`code-${i}`} className={`my-3 relative group rounded-lg overflow-hidden ${isDarkTheme
              ? 'chat-code-block'
              : 'bg-white/60 backdrop-blur-md border border-gray-300'
            }`}>
            <div className={`flex justify-between items-center px-3 py-2 ${isDarkTheme
                ? 'chat-code-header'
                : 'bg-white/80 backdrop-blur-sm border-b border-gray-300'
              }`}>
              <span className={`text-xs font-mono ${isDarkTheme ? 'text-white/70' : 'text-gray-600'
                }`}>{codeLanguage}</span>
              <button
                onClick={() => copyToClipboard(codeContent)}
                className={`text-xs transition-colors px-2 py-1 rounded ${isDarkTheme
            ...
            <div className="overflow-x-auto">
              <SyntaxHighlighter
                ...
                customStyle={{
                  margin: 0,
                  padding: '12px',
                  background: isDarkTheme ? 'rgba(0, 0, 0, 0.6)' : 'rgba(255, 255, 255, 0.8)',
                  fontSize: '11px',
                  lineHeight: '1.4'
                }}
                wrapLongLines={true}
    ...
    // Handle inline code WITHOUT copy functionality (just styling)
    const processInlineElements = (text: string): (string | JSX.Element)[] => {
      const parts: (string | JSX.Element)[] = [];
      let remaining = text;
      let keyCounter = 0;

      // Handle inline code - NO copy button, just styling
      remaining = remaining.replace(/`([^`]+)`/g, (code) => {
        const placeholder = `__INLINE_CODE_${keyCounter}__`;
        parts.push(
          <code key={`inline-code-${keyCounter++}`} className={`px-1.5 py-0.5 rounded text-xs font-mono border ${isDarkTheme
              ? 'bg-black/60 backdrop-blur-sm text-white/90 border-white/10'
      ...
      // Handle bold text
      remaining = remaining.replace(/\*\*(.*?)\*\*/g, (content) => {
        const placeholder = `__BOLD_${keyCounter}__`;
        parts.push(<strong key={`bold-${keyCounter++}`} className={isDarkTheme ? 'text-white/95' : 'text-gray-900'}>{content}</strong>);
        return placeholder;
      });
      ...
      remaining = remaining.replace(/(?<!\*)\*([^*]+)\*(?!\*)/g, (content) => {
        const placeholder = `__ITALIC_${keyCounter}__`;
        parts.push(<em key={`italic-${keyCounter++}`} className={isDarkTheme ? 'text-white/90' : 'text-gray-700'}>{content}</em>);
        return placeholder;
      });
    ...
    if (line.trim().startsWith('* ') || line.trim().startsWith('- ')) {
      ...
      elements.push(
        <div key={`bullet-${i}`} className="flex items-start gap-2 my-1">
          <span className={isDarkTheme ? 'text-white/60 mt-0.5' : 'text-gray-500 mt-0.5'}>•</span>
          <span className={isDarkTheme ? 'text-white/85' : 'text-gray-700'}>{processInlineElements(bulletContent)}</span>
        </div>
      );
      continue;
    }
    ...
    if (numberedMatch) {
      elements.push(
        <div key={`numbered-${i}`} className="flex items-start gap-2 my-1">
          <span className={isDarkTheme ? 'text-white/60 mt-0.5' : 'text-gray-500 mt-0.5'}>{numberedMatch[1]}.</span>
          <span className={isDarkTheme ? 'text-white/85' : 'text-gray-700'}>{processInlineElements(numberedMatch[2])}</span>
        </div>
      );
      continue;
    }
    ...
    if (line.startsWith('# ')) {
      elements.push(
        <h1 key={`h1-${i}`} className={`text-base font-bold my-2 ${isDarkTheme ? 'text-white/95' : 'text-gray-900'}`}>
          {processInlineElements(line.substring(2))}
        </h1>
      );
      continue;
    ...
    if (line.startsWith('## ')) {
      elements.push(
        <h2 key={`h2-${i}`} className={`text-sm font-semibold my-2 ${isDarkTheme ? 'text-white/90' : 'text-gray-800'}`}>
          {processInlineElements(line.substring(3))}
        </h2>
      );
      continue;
    ...
    if (line.startsWith('### ')) {
      elements.push(
        <h3 key={`h3-${i}`} className={`text-sm font-medium my-1 ${isDarkTheme ? 'text-white/85' : 'text-gray-700'}`}>
          {processInlineElements(line.substring(4))}
        </h3>
      );
      continue;
    ...
    // Regular paragraph
    elements.push(
      <p key={`p-${i}`} className={`my-1 ${isDarkTheme ? 'text-white/80' : 'text-gray-700'}`}>
        {processInlineElements(line)}
      </p>
    );
  }
  ...
  if (inCodeBlock && currentCodeBlock.length > 0) {
    const codeContent = currentCodeBlock.join('\n');
    elements.push(
      <div key="unclosed-code" className={`my-3 relative group rounded-lg overflow-hidden ${isDarkTheme
          ? 'chat-code-block'
          : 'bg-white/60 backdrop-blur-md border border-gray-300'
        }`}>
        <div className={`flex justify-between items-center px-3 py-2 ${isDarkTheme
...
const Queue: React.FC<QueueProps> = ({ setView, isDarkTheme, onThemeToggle }) => {
  const [toastOpen, setToastOpen] = useState(false)
  const [toastMessage, setToastMessage] = useState<ToastMessage>({
    title: "",
    description: "",
  ...
  // Enhanced screenshot handler with conversation context
  useEffect(() => {
    const unsubscribe = window.electronAPI.onScreenshotTaken(async (data: { path: string; preview: string } | { path: string; preview: string }[]) => {
      await refetch();
      setChatLoading(true);

      try {
        const latest = (data as { path: string })?.path || (Array.isArray(data) && data.length > 0 && data[data.length - 1]?.path);
        if (latest) {
          const conversationContext = chatMessages.length > 0
            ? `\n\nPrevious conversation context:\n${chatMessages.slice(-5).map(msg => `${msg.role}: ${msg.text}`).join('\n')}\n\n`
            : '';
  ...
  return (
    <div
      ...
      className={`select-none ${isDarkTheme ? '' : 'theme-light'}`}
    >
      <div className="bg-transparent w-full">
        <div className="px-2 py-1">
          <Toast
            ...
            duration={3000}
          >
            <ToastTitle isDarkTheme={isDarkTheme}>{toastMessage.title}</ToastTitle>
            <ToastDescription isDarkTheme={isDarkTheme}>{toastMessage.description}</ToastDescription>
          </Toast>
          <div className="w-fit">
            <QueueCommands
              screenshots={screenshots}
          ...
          {isChatOpen && (
            <div className={`mt-4 w-full mx-auto p-4 flex flex-col rounded-lg ${isDarkTheme
                ? 'liquid-glass chat-container'
                : 'bg-white/70 backdrop-blur-md border border-gray-300'
              }`}>
              {chatMessages.length === 0 ? (
                // ✅ No messages → only typing panel
                chatLoading && (
                  <div className="flex justify-start mb-3">
                    <div
                      className={`px-3 py-1.5 rounded-xl text-xs backdrop-blur-sm border shadow-md mr-12 ${isDarkTheme
                    ...
                    >
                      <span className="inline-flex items-center">
                        <span
                          className={`animate-pulse ${isDarkTheme ? "text-white/40" : "text-gray-400"
                            }`}
                        >
                          ●
                        </span>
                        <span
                          className={`animate-pulse animation-delay-200 ${isDarkTheme ? "text-white/40" : "text-gray-400"
                            }`}
                        >
                          ●
                        </span>
                        <span
                          className={`animate-pulse animation-delay-400 ${isDarkTheme ? "text-white/40" : "text-gray-400"
                            }`}
                        >
                          ●
                        </span>
                        <span
                          className={`ml-2 ${isDarkTheme ? "text-white/70" : "text-gray-600"
                            }`}
                        >
                          Gemini is thinking...
                        </span>
                ...
                // ✅ Messages exist → render messages + typing
                <div
                  ref={chatContainerRef}
                  className={`flex-1 overflow-y-auto mb-3 p-3 rounded-lg max-h-64 min-h-[120px] glass-content shadow-lg ${isDarkTheme
                      ? "chat-background scrollbar-dark"
                      : "bg-white/80 backdrop-blur-sm border border-gray-200 scrollbar-light"
                    }`}
                >
                  {chatMessages.map((msg, idx) => (
                    <div
                    ...
                    >
                      <div
                        className={`max-w-[80%] px-3 py-1.5 rounded-xl text-xs shadow-md backdrop-blur-sm border relative ${msg.role === "user"
                            ? isDarkTheme
                              ? "chat-message-user ml-12"
                              : "bg-blue-100 border-blue-200 text-blue-900 ml-12"
                            : isDarkTheme
                              ? "chat-message-assistant mr-12"
                              : "bg-white/90 border-gray-200 text-gray-800 mr-12"
                          }`}
                        style={{ wordBreak: "break-word", lineHeight: "1.4" }}
                      >
                        <button
                          onClick={() => copyMessageText(msg.text)}
                          className={`absolute -top-2 -right-2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-full text-xs ${isDarkTheme
                              ? "bg-black/80 hover:bg-black/90 text-white/80"
                        ...
                        {msg.role === "gemini" ? (
                          <div className="select-text">
                            {parseMarkdown(msg.text, isDarkTheme)}
                          </div>
                        ) : (
                          <div
                            className={`select-text ${isDarkTheme ? "text-white/90" : "text-blue-900"
                              }`}
                          >
                            {msg.text}
                          </div>
                  ...
                  {/* ✅ Typing indicator at bottom if still loading */}
                  {chatLoading && (
                    <div className="flex justify-start mb-3">
                      <div
                        className={`px-3 py-1.5 rounded-xl text-xs backdrop-blur-sm border shadow-md mr-12 ${isDarkTheme
              ...
              >
                <input
                  ref={chatInputRef}
                  className={`flex-1 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-1 shadow-lg transition-all duration-200 select-text ${isDarkTheme
                      ? 'chat-input focus:ring-white/30'
                      : 'bg-white/80 backdrop-blur-sm border border-gray-300 text-gray-800 placeholder-gray-500 focus:ring-blue-300'
                    }`}
                  placeholder="Type your message... (conversation history is maintained)"
                  value={chatInput}
                  onChange={e => setChatInput(e.target.value)}
                ...
                <button
                  type="submit"
                  className={`p-2 rounded-lg flex items-center justify-center transition-all duration-200 backdrop-blur-sm shadow-lg disabled:opacity-50 ${isDarkTheme
              ...
              <div className={`mt-2 flex justify-between items-center text-xs ${isDarkTheme ? 'text-white/50' : 'text-gray-400'
                }`}>
                <span>{chatMessages.length} messages</span>
                <button
                  onClick={() => {
                  ...
                  className={`transition-colors ${isDarkTheme ? 'text-white/40 hover:text-white/70' : 'text-gray-400 hover:text-gray-700'
                    }`}
                >
                  Clear Chat
                </button>
